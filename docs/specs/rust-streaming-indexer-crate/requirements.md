...
evaluated adaptive planning boundary within one completed planning pass.
...
- deterministic planning progress or quality information for the caller-visible
  ...
  work
- when the adaptive aggregate built-in realization is selected, deterministic
  ...
  boundary, and the adaptive boundary position of the first switch when one
  occurred
...
- when adaptive aggregate built-in planning is active for hierarchy planning,
  ...
  switch boundary, and the adaptive boundary position when that boundary has
  been evaluated
