...
use support::{MockAzureServer, run_in_tokio_runtime};
...
fn downstream_crates_can_run_the_contract_suite() {
    run_in_tokio_runtime(run_contract_suite(&AzureHarness::default())).unwrap();
}

#[test]
fn downstream_crates_can_run_the_full_suite() {
    run_in_tokio_runtime(run_full_suite(&AzureHarness::default())).unwrap();
}
