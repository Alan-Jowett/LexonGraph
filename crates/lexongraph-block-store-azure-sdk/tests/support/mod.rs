...
fn handle_connection(stream: TcpStream, inner: &Arc<MockAzureServerInner>) {
    ...
    let decoded_target = if query.is_empty() {
        decoded_path.clone()
    } else {
    ...
    let is_list = decoded_path == format!("/{}", inner.container_name)
    ...
    if !decoded_path.starts_with(&blob_prefix) {
        write_response(stream, 404, &[]);
        return;
    }
    let blob_name = decoded_path[blob_prefix.len()..].to_string();
...
fn respond_to_list(stream: TcpStream, inner: &Arc<MockAzureServerInner>) {
    ...
    names.sort();
    let body = render_listing(&inner.base_url, &inner.container_name, &names);
    let request_id = mock_request_id("LIST", "container");
    let headers = [("x-ms-request-id", request_id.as_str())];
    write_response_with_headers(stream, 200, &headers, body.as_bytes());
}

fn render_listing(base_url: &str, container_name: &str, names: &[String]) -> String {
    let blobs = names
        .iter()
    ...
    format!(
        concat!(
            ...
            "<EnumerationResults ServiceEndpoint=\"{base_url}\" ContainerName=\"{container_name}\">",
            "<Prefix></Prefix><Marker></Marker><MaxResults>5000</MaxResults>",
            "<Blobs>{blobs}</Blobs><NextMarker></NextMarker></EnumerationResults>"
        ),
        base_url = xml_escape(base_url),
        container_name = xml_escape(container_name),
        blobs = blobs
    )
}
...
mod tests {
    use azure_core::xml;
    use azure_storage_blob::models::ListBlobsResponse;

    use super::render_listing;

    #[test]
    fn listing_xml_matches_the_sdk_shape() {
        let body = render_listing(
            "http://127.0.0.1:10000/container?sv=test&sig=fake",
            "container",
            &[
                "aa/bb/cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc.cbor"
                    .into(),
            ],
        );
        let listing: ListBlobsResponse = xml::from_xml(body.as_bytes()).unwrap();
        assert_eq!(listing.blob_items.len(), 1);
        assert_eq!(
            listing.blob_items[0].name.as_deref(),
            Some("aa/bb/cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc.cbor")
        );
    }
}
