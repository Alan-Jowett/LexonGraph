...
pub(crate) fn solve_lexicographic_assignment(
    ...
    .ok_or(DcbcError::AssignmentInfeasible)?;
...
fn solve_subproblem(
    ...
    max_cluster_size: usize,
    fixed: &[Option<usize>],
) -> Result<Option<SubproblemSolution>, DcbcError> {
    ...
    let true_cost = assignment_cost(distances, cluster_count, &assignment)?;
    ...
    fn send_flow(
        ...
        while sent < target_flow {
                ...
                for edge in &self.graph[node] {
                    ...
                    if dist[node] + edge.cost + crate::EPSILON < dist[edge.to] {
                        return Err(DcbcError::InvalidNumericState(
                            "negative-cost residual cycle detected in assignment solver".into(),
                        ));
                    }
                }
            }
            ...
            while node != source {
                ...
                if steps > node_count {
                    return Err(DcbcError::InvalidNumericState(
                        "assignment solver predecessor chain became cyclic".into(),
                    ));
                }
